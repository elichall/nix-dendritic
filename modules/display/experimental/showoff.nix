...
# ==========================================================================
{ inputs, ... }: {
  flake.modules.homeManager.showoff = { pkgs, terminalName, ... }:
    let
    ...
    in
  {
        ...
        (writeShellApplication {
          ...
          text = ''
            ...
            kill_showoff() {
              ...
              if pgrep -f "$TERMINAL $CLASS_FLAG=showoff\." >/dev/null; then
                ...
                pkill -f "$TERMINAL $CLASS_FLAG=showoff\."
            ...
            hyprctl dispatch "hl.dsp.exec_cmd(\"$TERMINAL $CLASS_FLAG=showoff.dash -e showoff-layout\", { workspace = \"$WORKSPACE_PRI\", monitor = \"$MON_PRIMARY\" })"

            # If a secondary display is connected, spawn the rotator maximized on its own special workspace
            if [ "$MON_COUNT" -gt 1 ]; then
              hyprctl dispatch "hl.dsp.exec_cmd(\"$TERMINAL $CLASS_FLAG=showoff.sec -e term-rotator\", { workspace = \"$WORKSPACE_SEC\", monitor = \"$MON_SECONDARY\" })"
            fi
    ...
    xdg.configFile = {
      ...
      "tmux/showoff.conf".text = ''
        ...
        # Maps 24-bit color support to the external Wayland surface.
